...
L.Playback.Track = L.Class.extend({
        ...
        initialize : function (geoJSON, options) {
            options = options || {};
            var tickLen = options.tickLen || 1000;
...
hslToRgb: function (h, s, l){
    ...
    tock : function (timestamp, transitionTime) {
        ...
        for (var i = 0, len = this._tracks.length; i < len; i++) {
            ...
            ls.push([lngLat[1], lngLat[0]]);
            var latLng = new L.LatLng(lngLat[1], lngLat[0]);
            latLngs[i] = latLng;
            this._tracks[i].moveMarker(latLng, transitionTime,timestamp);
            if (i > 0)
            {
               length = length + Math.sqrt((lngLat[1] - lastLngLat[1])*(lngLat[1] - lastLngLat[1]) + (lngLat[0] - lastLngLat[0])*(lngLat[0] - lastLngLat[0]));
            }
            lastLngLat = lngLat;
        }
        ...
        L.polyline(ls, {color : color, opacity: opacity, weight: 1}).addTo(this._map);
    },
